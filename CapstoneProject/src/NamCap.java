...
/**
 * 
 * naM-caP is the main antagonist of the game, whose goal is to eat kiwis to
 * gain the ability to hunt the player down.
 * 
...
public class NamCap {
	...
	/**
	 * If naM-caP eats a kiwi, for the next 5 seconds, naM-caP will try to hunt the
	 * player. Otherwise, naM-caP will try and travel to the nearest kiwi.
	 * 
	 ...
	 * @return A Coordinate of the Kiwi's location with parent Coordinates that form
	 *         a path.
	 */
	 ...
	 * @param grid   The Map.
	 * @param player The Player.
	 * @return A Coordinate of the Player's location with parent Coordinates that
	 *         form a path.
	 */
	 ...
	 * @param grid   The Map.
	 * @param player The Player.
	 * @return true if the location is in the boundaries of the Map and it contains
	 *         either a path, or strawberry.
	 */
	public boolean isFree(int x, int y, Map grid, Player player) {
		return ((x >= 0 && x < grid.getLength()) && (y >= 0 && y < grid.getRowLength(x))
				&& (grid.get(x, y) == '.' || grid.get(x, y) == 'k' || grid.get(x, y) == 's' || grid.get(x, y) == '*'));
	}
	 ...
	 * @param player The Player.
	 * @return true if the location is in the boundaries of the Map and it contains
	 *         either a path, strawberry, or Player.
	 */
